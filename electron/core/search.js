      ...
      const entries = Object.values(group.entries).map((entry) => {
        const typeChoices = Object.keys(entry.papersByType)
          ...
          .map((type) => ({
            ...
            papers: this.sortBySourcePriority(
              entry.papersByType[type],
              manifest,
            ),
            sourceCount: new Set(
  ...
  rankSeedStandardCandidates(parsedQuery, limit = 3) {
    ...
    if (parsedQuery.level) {
      filtered = filtered.filter(
        (row) =>
          normaliseLevelValue(row.level) ===
          normaliseLevelValue(parsedQuery.level),
      );
    }
    ...
    const fuseCandidates = new Fuse(filtered, {
      ...
      .sort((a, b) => {
        ...
        return String(a.item.standardId).localeCompare(
          String(b.item.standardId),
        );
      });
