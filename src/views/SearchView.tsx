...
export default function SearchView({
	...
	const handleSelectStandard = async (std: SearchStandard) => {
		...
		const unsubscribe = window.ncea.onPapersProgress((progress) => {
			if (progress?.error || progress?.timeout) {
				console.warn(
					`[IPC:get-papers] adapter=${String(progress.adapter || "unknown")} timeout=${Boolean(progress.timeout)} error=${Boolean(progress.error)} message=${String(progress.message || "")}`
				);
			}
	...
	useEffect(() => {
		if (!papersProgress?.error) return;
		const adapterName = papersProgress.adapter || "a source";
		toast.error(`${adapterName} failed to fetch`, {
			description:
				papersProgress.message ||
				`Fetching from ${adapterName} failed. Please retry or check your connection.`,
		});
	}, [papersProgress]);

	// Show a toast when an adapter is taking too long and offer a retry action
	useEffect(() => {
		if (!papersProgress?.timeout) return;
		const adapterName = papersProgress.adapter || "a source";
		if (!selectingStandardId) {
			toast.error(`${adapterName} is taking too long`, {
				description: `Fetching from ${adapterName} timed out. Try again later or check your sources.`,
			});
			return;
		}

		toast.error(`${adapterName} is taking too long`, {
			description: `Fetching from ${adapterName} timed out. Retry fetching papers for this standard.`,
			action: {
				label: "Retry",
				onClick: async () => {
					try {
						...
						const detailed = grouped.find(
							(g) => g.standardId === selectingStandardId
						);
						if (detailed && detailed.entries.length > 0) {
							onSelectStandard(detailed);
						} else {
							toast.warning("  =^•ᴥ•^= No papers found", {
								description: `Sorry, couldn't find anything for ${selectingStandardId} from any configured source.`,
							});
						}
					} catch (e: unknown) {
	...
	}, [papersProgress, selectingStandardId, onSelectStandard]);
					...
					query && (
						<div className="rounded-lg border border-dashed border-muted-foreground/50 p-8 text-center">
							...
							<p className="text-xs text-muted-foreground mt-2">
								Try a different search term, or check that the right sources are enabled
								in Settings.
							</p>
							{searchSummary && (
								<p className="text-xs text-muted-foreground mt-2">{searchSummary}</p>
							)}
						</div>
					)}
				{!searchLoading &&
